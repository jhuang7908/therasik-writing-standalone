"""
Update Phylogenetic Trees with Detailed Labels (CDR3 Length + H2 Fold).
Colors: Red (Short CDR3 <= 10), Green (Long CDR3 > 10).
"""
...
def get_color(label_text, h3_len=None):
    ...
    if h3_len is not None:
        if h3_len <= 10: return "#d62728" # Red (Short)
        return "#2ca02c" # Green (Long)
...
def plot_tree(seqs, labels, h3_lens, title, filename):
    ...
    plt.figure(figsize=(14, 12)) # Wider figure for longer labels
    ax = plt.gca()
...
def main():
    ...
    # Detailed Labels: "Name (L=XX, H2-X-X)"
