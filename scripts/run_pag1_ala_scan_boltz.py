#!/usr/bin/env python
"""
PAG-1 Stage-2 full CDR alanine scan on RELAXED BOLTZ structures.
"""
...
OUT_DIR = ROOT / "projects/PAG project/vam_boltz_scan"
...
def _relaxed_pdb(clone_id: str) -> Path:
    ...
    if not pdb.is_file():
        raise FileNotFoundError(f"Missing relaxed PDB for {clone_id}: {pdb}")
    return pdb
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--clone",
        nargs="+",
        choices=sorted(RELAXED_PDBS),
        default=sorted(RELAXED_PDBS),
