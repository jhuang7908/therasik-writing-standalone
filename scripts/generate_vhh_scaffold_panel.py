...
"""
generate_vhh_scaffold_panel.py
...
INPUT_JSON = PROJECT_ROOT / "data" / "germlines" / "vicugna_pacos_ig_aa" / "vhh_numbered" / "vhh_numbered_and_split.json"
OUTPUT_DIR = PROJECT_ROOT / "data" / "germlines" / "vicugna_pacos_ig_aa" / "vhh_scaffolds"
OUTPUT_JSON = OUTPUT_DIR / "vhh_scaffolds.json"
OUTPUT_FASTA = OUTPUT_DIR / "vhh_scaffolds.fasta"
...
def load_vhh_segments(path: Path) -> List[Dict[str, Any]]:
    """VHH，FR"""
...
def main():
    ...
    records = load_vhh_segments(INPUT_JSON)
    print(f"  : {len(records)}")
