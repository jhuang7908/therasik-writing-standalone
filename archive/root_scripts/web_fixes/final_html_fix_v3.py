...
for card in cards:
    ...
    # Ensure all fields are present in the card detail
    detail_start = new_card.find('<div class="cc-detail">')
    detail_end = new_card.find('</div>\n  </div>\n</div>', detail_start)
    if detail_start != -1 and detail_end != -1:
        detail_section = new_card[detail_start:detail_end]
        
        # Check for missing rows and add them if necessary
        for label, prop_key in field_map.items:
            if f'<span class="info-label">{label}</span>' not in detail_section:
                val = props.get(prop_key, "NA")
                new_row = f'<div class="info-row"><span class="info-label">{label}</span><span class="info-value">{val}</span></div>'
                # Insert before Design Logic if it exists, otherwise at the end
                logic_marker = ' (Design Logic)'
                if logic_marker in detail_section:
                    logic_pos = detail_section.find(f'<div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.6px;color:#888;margin:10px 0 4px;border-top:1px solid #eee;padding-top:8px">{logic_marker}')
                    if logic_pos != -1:
                        detail_section = detail_section[:logic_pos] + new_row + detail_section[logic_pos:]
                else:
                    detail_section += new_row
        
        # Update values in detail_section
        for label, prop_key in field_map.items:
            if prop_key in props:
                detail_section = update_info_row(detail_section, label, props[prop_key])
        
        new_card = new_card[:detail_start] + detail_section + new_card[detail_end:]

    updated_cards.append(new_card)

new_grid_html = "\n".join([c.strip for c in updated_cards if c.strip])
...
print("Final comprehensive fix with search, layout, and missing fields completed.")
