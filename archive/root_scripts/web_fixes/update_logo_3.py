...
for fname in ['thanks.html', 'thanks_zh.html', 'ada_database.html', 'InSynBio_Showcase.html', '404.html']:
    path = os.path.join(BASE, fname)
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            content = f.read()
        
        new_content = re.sub(r'(<a href="index\.html"[^>]*>\s*)<svg[^>]*>.*?</svg>', r'\1' + NEW_LOGO, content, flags=re.DOTALL)
