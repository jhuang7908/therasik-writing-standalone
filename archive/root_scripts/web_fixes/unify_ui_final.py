...
# 1. UNIFY ALL DROPDOWNS TO ENGLISH
# Payload Mechanisms
...
content = content.replace('<option value="">Solid Tumor</option>', '<option value="">Solid Tumor</option>') # Value is still ZH for JS logic
...
# 2. FIX LINKER TYPES (Especially pH-cleavable)
def fix_linker_cards(content):
    ...
    def update_card(match):
        ...
        elif any(x in title.lower for x in ["hydrazone", "acid-labile", "acbut", "ph-cleavable"]):
            ltype = "pH-cleavable"
        ...
        if 'background:#f3f4f6;color:#666;border:1px solid #ddd' in card_html:
            if "Public Domain" in card_html:
                ...
                # Everything else gets a light green "Protected" look
...
def color_all_patents(content):
    # This is a broader sweep for Payload and Conjugation too
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Unified dropdowns to English. Fixed pH-cleavable cards. Colored all non-public patent badges.")
