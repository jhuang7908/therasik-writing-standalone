...
def fix_linkers(content):
    ...
    def update_card(match):
        ...
        # Patent coloring (already done but ensuring consistency)
        if "Daiichi Sankyo" in card_html:
            card_html = re.sub(r'background:#f3f4f6;color:#666;border:1px solid #ddd', 'background:#fff3cd;color:#856404;border:1px solid #ffeeba', card_html)
        elif "Seagen" in card_html:
            card_html = re.sub(r'background:#f3f4f6;color:#666;border:1px solid #ddd', 'background:#cce5ff;color:#004085;border:1px solid #b8daff')
        elif "Spirogen" in card_html or "AstraZeneca" in card_html:
            card_html = re.sub(r'background:#f3f4f6;color:#666;border:1px solid #ddd', 'background:#cce5ff;color:#004085;border:1px solid #b8daff')
        elif "Synaffix" in card_html or "Ambrx" in card_html or "Protected" in card_html:
            card_html = re.sub(r'background:#f3f4f6;color:#666;border:1px solid #ddd', 'background:#d4edda;color:#155724;border:1px solid #c3e6cb')
...
# 2. Fix Dropdown Localization (Ensuring English values for sync, but keeping Chinese for user clarity)
def fix_dropdowns(content):
    # Linker dropdown already has Chinese in labels, ensuring values are English
    # Payload dropdown: ensure values are English, labels have Chinese
