...
batch4_path = r'd:\InSynBio-AI-Research\Antibody_Engineer_Suite\data\immunogenicity_knowledge_base\master\ada_batch4_expanded.csv'
...
batch4_df = pd.read_csv(batch4_path)
ph3_df = pd.read_csv(ph3_candidates_path)

# Extract 10 selected candidates from ph3_df for metadata
selected_names = batch4_df['antibody_name'].tolist()
metadata_df = ph3_df[ph3_df['antibody_name'].isin(selected_names)].copy()

# Create a mapping for batch4 data
batch4_map = batch4_df.set_index('antibody_name').to_dict('index')
...
for name in selected_names:
    ...
    # Get research data from batch4_df
    research = batch4_map[name]
