...
batch7_path = r'd:\InSynBio-AI-Research\Antibody_Engineer_Suite\data\immunogenicity_knowledge_base\master\ada_batch7_expanded.csv'
...
batch7_df = pd.read_csv(batch7_path)
ph3_df = pd.read_csv(ph3_candidates_path)

# Extract selected candidates from ph3_df for metadata
selected_names = batch7_df['antibody_name'].tolist()
metadata_df = ph3_df[ph3_df['antibody_name'].isin(selected_names)].copy()

# Create a mapping for batch7 data
batch7_map = batch7_df.set_index('antibody_name').to_dict('index')
...
for name in selected_names:
    ...
    # Get research data from batch7_df
    research = batch7_map[name]
