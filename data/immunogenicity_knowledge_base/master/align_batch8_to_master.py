...
batch8_path = r'd:\InSynBio-AI-Research\Antibody_Engineer_Suite\data\immunogenicity_knowledge_base\master\ada_batch8_expanded.csv'
...
batch8_df = pd.read_csv(batch8_path)
ph3_df = pd.read_csv(ph3_candidates_path)

# Extract selected candidates from ph3_df for metadata
selected_names = batch8_df['antibody_name'].tolist()
metadata_df = ph3_df[ph3_df['antibody_name'].isin(selected_names)].copy()

# Create a mapping for batch8 data
batch8_map = batch8_df.set_index('antibody_name').to_dict('index')
...
for name in selected_names:
    ...
    # Get research data from batch8_df
    research = batch8_map[name]
