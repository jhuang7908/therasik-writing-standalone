...
batch5_path = r'd:\InSynBio-AI-Research\Antibody_Engineer_Suite\data\immunogenicity_knowledge_base\master\ada_batch5_expanded.csv'
...
batch5_df = pd.read_csv(batch5_path)
ph3_df = pd.read_csv(ph3_candidates_path)

# Extract selected candidates from ph3_df for metadata
selected_names = batch5_df['antibody_name'].tolist()
metadata_df = ph3_df[ph3_df['antibody_name'].isin(selected_names)].copy()

# Create a mapping for batch5 data
batch5_map = batch5_df.set_index('antibody_name').to_dict('index')
...
for name in selected_names:
    ...
    # Get research data from batch5_df
    research = batch5_map[name]
