"""Run cdr_physchem_distribution for VHH29."""
import sys, os
...
csv_path = Path('data/vhh_database_b_union/vhh29_cdr_segments.csv')
loci = {'vhh_cdr1': 'CDR1', 'vhh_cdr2': 'CDR2', 'vhh_cdr3': 'CDR3'}
id_col = 'safe_id'
label = 'VHH29 (humanized VHH, Database B)'
cdr_def = 'IMGT (pyhmmer/anarci, 2026-04-30)'
output = Path('data/reference/CDR_physchem_VHH29_v1.json')
...
# Print summary
for locus, block in loci_blocks.items():
    n = block['n']
    if n == 0:
        print(f'  {locus}: n=0', flush=True)
        continue
    m = block['metrics']
    cdr3_len = m.get('length', {})
    gravy = m.get('gravy', {})
    print(f'  {locus}: n={n}, len_median={cdr3_len.get("p50","?")}, gravy_median={gravy.get("p50","?")}', flush=True)
